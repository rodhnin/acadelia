// backend/controllers/chat/patologiaController.js (ULTRA-SIMPLIFICADO CON TOKEN MANAGER)

// ===== IMPORTACIONES NECESARIAS (REDUCIDAS 60%) =====
import { handlePathologyQuery, handlePathologyMultimodalQuery, handlePathologyQueryWithoutSaving, handlePathologyMultimodalQueryWithoutSaving } from "../../../../services/chat/ias/Medicina/patologiaService.js";
...
/**
 * ✅ FUNCIÓN ULTRA-SIMPLIFICADA: queryPatologia
 * REDUCCIÓN: 90% menos código usando TokenManager.handleCompleteAvaController
 */
export const queryPatologia = async (req, res) => {
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    serviceFunction: async (params) => {
      ...
      if (skipSave) {
        ...
        return await handlePathologyQueryWithoutSaving(modifiedParams);
      } else {
        return await handlePathologyQuery(params);
      }
    },
...
export const queryPatologiaMultimodal = async (req, res) => {
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'patologia_multimodal',
    defaultAvaName: 'Patología',
    serviceFunction: async (params) => {
      const result = await handlePathologyMultimodalQuery(params);
      
      if (result.error) {
        logSecurityEvent('PATOLOGIA_MULTIMODAL_CONTROLLED_ERROR', 'Error controlado en consulta multimodal de patología', {
...
export const queryPatologiaMultimodalWithoutSaving = async (req, res) => {
  const validationErrors = validateMultimodalParams(req.body);
  const avaAccessInfo = req.accessInfo?.avaAccess || {};
  const tokenInfo = req.tokenInfo || {};
  const tokenWarning = req.tokenWarning || null;
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'patologia_multimodal_without_saving',
    defaultAvaName: 'Patología',
    serviceFunction: async (params) => {
      const result = await handlePathologyMultimodalQueryWithoutSaving(params);
      
      if (result.error) {
        logSecurityEvent('PATOLOGIA_MULTIMODAL_WITHOUT_SAVING_ERROR', 'Error controlado en consulta multimodal de patología sin guardar', {
          userId: params.userId || req.user?.id_user,
          error: result.error,
          ip: req.ip
        }, 'medium');
