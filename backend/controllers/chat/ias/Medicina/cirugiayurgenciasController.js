// backend/controllers/chat/cirugiayurgenciasController.js (ULTRA-SIMPLIFICADO CON TOKEN MANAGER)

// ===== IMPORTACIONES NECESARIAS (REDUCIDAS 60%) =====
import { handleSurgeryAndEmergencyQuery, handleSurgeryAndEmergencyMultimodalQuery, handleSurgeryAndEmergencyQueryWithoutSaving, handleSurgeryAndEmergencyMultimodalQueryWithoutSaving } from "../../../../services/chat/ias/Medicina/cirugiayurgenciasService.js";
...
/**
 * ✅ FUNCIÓN ULTRA-SIMPLIFICADA: queryCirugiaYUrgencias
 * REDUCCIÓN: 90% menos código usando TokenManager.handleCompleteAvaController
 */
export const queryCirugiaYUrgencias = async (req, res) => {
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'cirugiayurgencias',
    defaultAvaName: 'Cirugía y Urgencias',
    serviceFunction: async (params) => {
      ...
      if (skipSave) {
        ...
        return await handleSurgeryAndEmergencyQueryWithoutSaving(modifiedParams);
      } else {
        return await handleSurgeryAndEmergencyQuery(params);
      }
    },
...
export const queryCirugiaYUrgenciasMultimodal = async (req, res) => {
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'cirugiayurgencias_multimodal',
    defaultAvaName: 'Cirugía y Urgencias',
    serviceFunction: async (params) => {
      const result = await handleSurgeryAndEmergencyMultimodalQuery(params);
      
      if (result.error) {
        logSecurityEvent('CIRUGIA_URGENCIAS_MULTIMODAL_CONTROLLED_ERROR', 'Error controlado en consulta multimodal de cirugía y urgencias', {
...
export const queryCirugiaYUrgenciasMultimodalWithoutSaving = async (req, res) => {
  const validationErrors = validateMultimodalParams(req.body);
  const avaAccessInfo = req.accessInfo?.avaAccess || {};
  const tokenInfo = req.tokenInfo || {};
  const tokenWarning = req.tokenWarning || null;
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'cirugiayurgencias_multimodal_without_saving',
    defaultAvaName: 'Cirugía y Urgencias',
    serviceFunction: async (params) => {
      const result = await handleSurgeryAndEmergencyMultimodalQueryWithoutSaving(params);
      
      if (result.error) {
        logSecurityEvent('CIRUGIA_URGENCIAS_MULTIMODAL_WITHOUT_SAVING_ERROR', 'Error controlado en consulta multimodal de cirugía y urgencias sin guardar', {
          userId: params.userId || req.user?.id_user,
          error: result.error,
          ip: req.ip
        }, 'medium');
