// backend/controllers/chat/cienciasaplicadasController.js (ULTRA-SIMPLIFICADO CON TOKEN MANAGER)

// ===== IMPORTACIONES NECESARIAS (REDUCIDAS 60%) =====
import { handleAppliedSciencesQuery, handleAppliedSciencesMultimodalQuery, handleAppliedSciencesQueryWithoutSaving, handleAppliedSciencesMultimodalQueryWithoutSaving } from "../../../../services/chat/ias/Medicina/cienciasaplicadasService.js";
...
/**
 * ✅ FUNCIÓN ULTRA-SIMPLIFICADA: queryCienciasAplicadas
 * REDUCCIÓN: 90% menos código usando TokenManager.handleCompleteAvaController
 */
export const queryCienciasAplicadas = async (req, res) => {
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'cienciasaplicadas',
    defaultAvaName: 'Ciencias Básicas Aplicadas',
    serviceFunction: async (params) => {
      ...
      if (skipSave) {
        ...
        return await handleAppliedSciencesQueryWithoutSaving(modifiedParams);
      } else {
        return await handleAppliedSciencesQuery(params);
      }
    },
...
export const queryCienciasAplicadasMultimodal = async (req, res) => {
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'cienciasaplicadas_multimodal',
    defaultAvaName: 'Ciencias Básicas Aplicadas',
    serviceFunction: async (params) => {
      const result = await handleAppliedSciencesMultimodalQuery(params);
      
      if (result.error) {
        logSecurityEvent('CIENCIASAPLICADAS_MULTIMODAL_CONTROLLED_ERROR', 'Error controlado en consulta multimodal de ciencias aplicadas', {
...
export const queryCienciasAplicadasMultimodalWithoutSaving = async (req, res) => {
  const validationErrors = validateMultimodalParams(req.body);
  const avaAccessInfo = req.accessInfo?.avaAccess || {};
  const tokenInfo = req.tokenInfo || {};
  const tokenWarning = req.tokenWarning || null;
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'cienciasaplicadas_multimodal_without_saving',
    defaultAvaName: 'Ciencias Básicas Aplicadas',
    serviceFunction: async (params) => {
      const result = await handleAppliedSciencesMultimodalQueryWithoutSaving(params);
      
      if (result.error) {
        logSecurityEvent('CIENCIASAPLICADAS_MULTIMODAL_WITHOUT_SAVING_ERROR', 'Error controlado en consulta multimodal de ciencias aplicadas sin guardar', {
          userId: params.userId || req.user?.id_user,
          error: result.error,
          ip: req.ip
        }, 'medium');
