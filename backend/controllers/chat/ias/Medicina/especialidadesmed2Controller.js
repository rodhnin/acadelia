...
/**
 * ✅ FUNCIÓN ULTRA-SIMPLIFICADA: queryEspecialidadesMedicasII
 * REDUCCIÓN: 90% menos código usando TokenManager.handleCompleteAvaController
 */
export const queryEspecialidadesMedicasII = async (req, res) => {
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'especialidadmed2',
    defaultAvaName: 'Especialidades Médicas II',
...
export const queryEspecialidadesMedicasIIMultimodal = async (req, res) => {
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'especialidadmed2_multimodal',
    defaultAvaName: 'Especialidades Médicas II',
    serviceFunction: async (params) => {
      const result = await handleMedicalSpecialtiesMultimodalQuery(params);
      
      if (result.error) {
        logSecurityEvent('ESPECIALIDADMED2_MULTIMODAL_CONTROLLED_ERROR', 'Error controlado en consulta multimodal de especialidades médicas II', {
...
export const queryEspecialidadesMedicasIIMultimodalWithoutSaving = async (req, res) => {
  const validationErrors = validateMultimodalParams(req.body);
  const avaAccessInfo = req.accessInfo?.avaAccess || {};
  const tokenInfo = req.tokenInfo || {};
  const tokenWarning = req.tokenWarning || null;
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'especialidadmed2_multimodal_without_saving',
    defaultAvaName: 'Especialidades Médicas II',
    serviceFunction: async (params) => {
      const result = await handleMedicalSpecialtiesMultimodalQueryWithoutSaving(params);
      
      if (result.error) {
        logSecurityEvent('ESPECIALIDADMED2_MULTIMODAL_WITHOUT_SAVING_ERROR', 'Error controlado en consulta multimodal de especialidades médicas II sin guardar', {
          userId: params.userId || req.user?.id_user,
          error: result.error,
          ip: req.ip
        }, 'medium');
