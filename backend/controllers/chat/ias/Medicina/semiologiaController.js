// backend/controllers/chat/semiologiaController.js (ULTRA-SIMPLIFICADO CON TOKEN MANAGER)

// ===== IMPORTACIONES NECESARIAS (REDUCIDAS 60%) =====
import { handleSemiologyQuery, handleSemiologyMultimodalQuery, handleSemiologyQueryWithoutSaving, handleSemiologyMultimodalQueryWithoutSaving } from "../../../../services/chat/ias/Medicina/semiologiaService.js";
...
/**
 * ✅ FUNCIÓN ULTRA-SIMPLIFICADA: querySemiologia
 * REDUCCIÓN: 90% menos código usando TokenManager.handleCompleteAvaController
 */
export const querySemiologia = async (req, res) => {
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenWarning,
    avaType: 'semiologia',
    defaultAvaName: 'Semiología y Diagnóstico',
    serviceFunction: async (params) => {
      ...
      if (skipSave) {
        ...
        return await handleSemiologyQueryWithoutSaving(modifiedParams);
      } else {
        return await handleSemiologyQuery(params);
      }
    },
...
export const querySemiologiaMultimodal = async (req, res) => {
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'semiologia_multimodal',
    defaultAvaName: 'Semiología y Diagnóstico',
    serviceFunction: async (params) => {
      const result = await handleSemiologyMultimodalQuery(params);
      
      if (result.error) {
        logSecurityEvent('SEMIOLOGIA_MULTIMODAL_CONTROLLED_ERROR', 'Error controlado en consulta multimodal de semiología', {
...
export const querySemiologiaMultimodalWithoutSaving = async (req, res) => {
  const validationErrors = validateMultimodalParams(req.body);
  const avaAccessInfo = req.accessInfo?.avaAccess || {};
  const tokenInfo = req.tokenInfo || {};
  const tokenWarning = req.tokenWarning || null;
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'semiologia_multimodal_without_saving',
    defaultAvaName: 'Semiología y Diagnóstico',
    serviceFunction: async (params) => {
      const result = await handleSemiologyMultimodalQueryWithoutSaving(params);
      
      if (result.error) {
        logSecurityEvent('SEMIOLOGIA_MULTIMODAL_WITHOUT_SAVING_ERROR', 'Error controlado en consulta multimodal de semiología sin guardar', {
          userId: params.userId || req.user?.id_user,
          error: result.error,
          ip: req.ip
        }, 'medium');
