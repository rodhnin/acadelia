// backend/controllers/chat/medicinainternaController.js (ULTRA-SIMPLIFICADO CON TOKEN MANAGER)

// ===== IMPORTACIONES NECESARIAS (REDUCIDAS 60%) =====
import { handleInternalMedicineQuery, handleInternalMedicineMultimodalQuery, handleInternalMedicineQueryWithoutSaving, handleInternalMedicineMultimodalQueryWithoutSaving } from "../../../../services/chat/ias/Medicina/medicinainternaService.js";
...
/**
 * ✅ FUNCIÓN ULTRA-SIMPLIFICADA: queryMedicinaInterna
 * REDUCCIÓN: 90% menos código usando TokenManager.handleCompleteAvaController
 */
export const queryMedicinaInterna = async (req, res) => {
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'medicinainterna',
    defaultAvaName: 'Medicina Interna',
    serviceFunction: async (params) => {
      ...
      if (skipSave) {
        ...
        return await handleInternalMedicineQueryWithoutSaving(modifiedParams);
      } else {
        return await handleInternalMedicineQuery(params);
      }
    },
...
export const queryMedicinaInternaMultimodal = async (req, res) => {
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'medicinainterna_multimodal',
    defaultAvaName: 'Medicina Interna',
    serviceFunction: async (params) => {
      const result = await handleInternalMedicineMultimodalQuery(params);
      
      if (result.error) {
        logSecurityEvent('MEDICINA_INTERNA_MULTIMODAL_CONTROLLED_ERROR', 'Error controlado en consulta multimodal de medicina interna', {
...
export const queryMedicinaInternaMultimodalWithoutSaving = async (req, res) => {
  const validationErrors = validateMultimodalParams(req.body);
  const avaAccessInfo = req.accessInfo?.avaAccess || {};
  const tokenInfo = req.tokenInfo || {};
  const tokenWarning = req.tokenWarning || null;
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'medicinainterna_multimodal_without_saving',
    defaultAvaName: 'Medicina Interna',
    serviceFunction: async (params) => {
      const result = await handleInternalMedicineMultimodalQueryWithoutSaving(params);
      
      if (result.error) {
        logSecurityEvent('MEDICINA_INTERNA_MULTIMODAL_WITHOUT_SAVING_ERROR', 'Error controlado en consulta multimodal de medicina interna sin guardar', {
          userId: params.userId || req.user?.id_user,
          error: result.error,
          ip: req.ip
        }, 'medium');
