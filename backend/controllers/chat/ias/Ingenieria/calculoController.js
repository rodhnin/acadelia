...
/**
 * ✅ FUNCIÓN ULTRA-SIMPLIFICADA: queryCalculo
 ...
 */
export const queryCalculo = async (req, res) => {
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    serviceFunction: async (params) => {
      ...
      if (skipSave) {
        ...
        return await handleCalculusQueryWithoutSaving(modifiedParams);
      } else {
        return await handleCalculusQuery(params);
      }
    },
...
export const queryCalculoMultimodal = async (req, res) => {
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'calculo_multimodal',
    defaultAvaName: 'Cálculo',
    serviceFunction: async (params) => {
      const result = await handleCalculusMultimodalQuery(params);
      
      if (result.error) {
        logSecurityEvent('CALCULO_MULTIMODAL_CONTROLLED_ERROR', 'Error controlado en consulta multimodal de cálculo', {
...
export const queryCalculoMultimodalWithoutSaving = async (req, res) => {
  const validationErrors = validateMultimodalParams(req.body);
  const avaAccessInfo = req.accessInfo?.avaAccess || {};
  const tokenInfo = req.tokenInfo || {};
  const tokenWarning = req.tokenWarning || null;
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'calculo_multimodal_without_saving',
    defaultAvaName: 'Cálculo',
    serviceFunction: async (params) => {
      const result = await handleCalculusMultimodalQueryWithoutSaving(params);
      
      if (result.error) {
        logSecurityEvent('CALCULO_MULTIMODAL_WITHOUT_SAVING_ERROR', 'Error controlado en consulta multimodal de cálculo sin guardar', {
          userId: params.userId || req.user?.id_user,
          error: result.error,
          ip: req.ip
        }, 'medium');
