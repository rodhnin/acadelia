// backend/controllers/chat/MathematicsAdvancedController.js (ULTRA-SIMPLIFICADO CON TOKEN MANAGER)
...
export const queryMathematicsAdvanced = async (req, res) => {
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'matematicaavz',
    defaultAvaName: 'Matemáticas Avanzadas',
    serviceFunction: async (params) => {
      ...
      if (skipSave) {
        ...
        return await handleMathematicsAdvancedQueryWithoutSaving(modifiedParams);
      } else {
        return await handleMathematicsAdvancedQuery(params);
      }
    },
...
export const queryMathematicsAdvancedMultimodal = async (req, res) => {
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'matematicaavz_multimodal',
    defaultAvaName: 'Matemáticas Avanzadas',
    serviceFunction: async (params) => {
      const result = await handleMathematicsAdvancedMultimodalQuery(params);
      
      if (result.error) {
        logSecurityEvent('MATEMATICAAVZ_MULTIMODAL_CONTROLLED_ERROR', 'Error controlado en consulta multimodal de matemáticas avanzadas', {
...
export const queryMathematicsAdvancedMultimodalWithoutSaving = async (req, res) => {
  const validationErrors = validateMultimodalParams(req.body);
  const avaAccessInfo = req.accessInfo?.avaAccess || {};
  const tokenInfo = req.tokenInfo || {};
  const tokenWarning = req.tokenWarning || null;
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'matematicaavz_multimodal_without_saving',
    defaultAvaName: 'Matemáticas Avanzadas',
    serviceFunction: async (params) => {
      const result = await handleMathematicsAdvancedMultimodalQueryWithoutSaving(params);
      
      if (result.error) {
        logSecurityEvent('MATEMATICAAVZ_MULTIMODAL_WITHOUT_SAVING_ERROR', 'Error controlado en consulta multimodal de matemáticas avanzadas sin guardar', {
          userId: params.userId || req.user?.id_user,
          error: result.error,
          ip: req.ip
        }, 'medium');
