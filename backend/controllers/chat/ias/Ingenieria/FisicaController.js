// backend/controllers/chat/FisicaController.js (ULTRA-SIMPLIFICADO CON TOKEN MANAGER)

// ===== IMPORTACIONES NECESARIAS (REDUCIDAS 60%) =====
import { handlePhysicsQuery, handlePhysicsMultimodalQuery, handlePhysicsQueryWithoutSaving, handlePhysicsMultimodalQueryWithoutSaving } from "../../../../services/chat/ias/ingenieria/physicService.js";
...
/**
 * ✅ FUNCIÓN ULTRA-SIMPLIFICADA: queryFisica  
 * REDUCCIÓN: 95% menos código usando TokenManager.handleCompleteAvaController
 */
export const queryFisica = async (req, res) => {
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    serviceFunction: async (params) => {
      ...
      if (skipSave) {
        ...
        return await handlePhysicsQueryWithoutSaving(modifiedParams);
      } else {
        return await handlePhysicsQuery(params);
      }
    },
...
export const queryFisicaMultimodal = async (req, res) => {
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'fisica_multimodal',
    defaultAvaName: 'Física',
    serviceFunction: async (params) => {
      const result = await handlePhysicsMultimodalQuery(params);
      
      if (result.error) {
        logSecurityEvent('FISICA_MULTIMODAL_CONTROLLED_ERROR', 'Error controlado en consulta multimodal de física', {
...
export const queryFisicaMultimodalWithoutSaving = async (req, res) => {
  const validationErrors = validateMultimodalParams(req.body);
  const avaAccessInfo = req.accessInfo?.avaAccess || {};
  const tokenInfo = req.tokenInfo || {};
  const tokenWarning = req.tokenWarning || null;
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'fisica_multimodal_without_saving',
    defaultAvaName: 'Física',
    serviceFunction: async (params) => {
      const result = await handlePhysicsMultimodalQueryWithoutSaving(params);
      
      if (result.error) {
        logSecurityEvent('FISICA_MULTIMODAL_WITHOUT_SAVING_ERROR', 'Error controlado en consulta multimodal de física sin guardar', {
          userId: params.userId || req.user?.id_user,
          error: result.error,
          ip: req.ip
        }, 'medium');
