// backend/controllers/chat/ComputacionController.js (ULTRA-SIMPLIFICADO CON TOKEN MANAGER)

// ===== IMPORTACIONES NECESARIAS (REDUCIDAS 60%) =====
import { handleComputacionQuery, handleComputacionMultimodalQuery, handleComputacionQueryWithoutSaving, handleComputacionMultimodalQueryWithoutSaving } from "../../../../services/chat/ias/ingenieria/computacionService.js";
...
/**
 * ✅ FUNCIÓN ULTRA-SIMPLIFICADA: queryComputacion  
 * REDUCCIÓN: 95% menos código usando TokenManager.handleCompleteAvaController
 */
export const queryComputacion = async (req, res) => {
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenWarning,
    avaType: 'computacion',
    defaultAvaName: 'Computación y Sistemas',
    serviceFunction: async (params) => {
      ...
      if (skipSave) {
        ...
        return await handleComputacionQueryWithoutSaving(modifiedParams);
      } else {
        return await handleComputacionQuery(params);
      }
    },
...
export const queryComputacionMultimodal = async (req, res) => {
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'computacion_multimodal',
    defaultAvaName: 'Computación y Sistemas',
    serviceFunction: async (params) => {
      const result = await handleComputacionMultimodalQuery(params);
      
      if (result.error) {
        logSecurityEvent('COMPUTACION_MULTIMODAL_CONTROLLED_ERROR', 'Error controlado en consulta multimodal de computación', {
...
export const queryComputacionMultimodalWithoutSaving = async (req, res) => {
  const validationErrors = validateMultimodalParams(req.body);
  const avaAccessInfo = req.accessInfo?.avaAccess || {};
  const tokenInfo = req.tokenInfo || {};
  const tokenWarning = req.tokenWarning || null;
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'computacion_multimodal_without_saving',
    defaultAvaName: 'Computación y Sistemas',
    serviceFunction: async (params) => {
      const result = await handleComputacionMultimodalQueryWithoutSaving(params);
      
      if (result.error) {
        logSecurityEvent('COMPUTACION_MULTIMODAL_WITHOUT_SAVING_ERROR', 'Error controlado en consulta multimodal de computación sin guardar', {
          userId: params.userId || req.user?.id_user,
          error: result.error,
          ip: req.ip
        }, 'medium');
