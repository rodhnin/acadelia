// backend/controllers/chat/ResistenciaMaterialesController.js (ULTRA-SIMPLIFICADO CON TOKEN MANAGER)

// ===== IMPORTACIONES NECESARIAS (REDUCIDAS 60%) =====
import { handleResistenciaMaterialesQuery, handleResistenciaMaterialesMultimodalQuery, handleResistenciaMaterialesQueryWithoutSaving, handleResistenciaMaterialesMultimodalQueryWithoutSaving } from "../../../../services/chat/ias/ingenieria/ResistenciaMaterialesService.js";
...
/**
 * ✅ FUNCIÓN ULTRA-SIMPLIFICADA: queryResistenciaMateriales  
 * REDUCCIÓN: 95% menos código usando TokenManager.handleCompleteAvaController
 */
export const queryResistenciaMateriales = async (req, res) => {
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'resistencia_materiales',
    defaultAvaName: 'Mecánica y Resistencia de Materiales',
    serviceFunction: async (params) => {
      ...
      if (skipSave) {
        ...
        return await handleResistenciaMaterialesQueryWithoutSaving(modifiedParams);
      } else {
        return await handleResistenciaMaterialesQuery(params);
      }
    },
...
export const queryResistenciaMaterialesMultimodal = async (req, res) => {
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'resistencia_materiales_multimodal',
    defaultAvaName: 'Mecánica y Resistencia de Materiales',
    serviceFunction: async (params) => {
      const result = await handleResistenciaMaterialesMultimodalQuery(params);
      
      if (result.error) {
        logSecurityEvent('RESISTENCIA_MATERIALES_MULTIMODAL_CONTROLLED_ERROR', 'Error controlado en consulta multimodal de resistencia de materiales', {
...
export const queryResistenciaMaterialesMultimodalWithoutSaving = async (req, res) => {
  const validationErrors = validateMultimodalParams(req.body);
  const avaAccessInfo = req.accessInfo?.avaAccess || {};
  const tokenInfo = req.tokenInfo || {};
  const tokenWarning = req.tokenWarning || null;
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'resistencia_materiales_multimodal_without_saving',
    defaultAvaName: 'Mecánica y Resistencia de Materiales',
    serviceFunction: async (params) => {
      const result = await handleResistenciaMaterialesMultimodalQueryWithoutSaving(params);
      
      if (result.error) {
        logSecurityEvent('RESISTENCIA_MATERIALES_MULTIMODAL_WITHOUT_SAVING_ERROR', 'Error controlado en consulta multimodal de resistencia de materiales sin guardar', {
          userId: params.userId || req.user?.id_user,
          error: result.error,
          ip: req.ip
        }, 'medium');
