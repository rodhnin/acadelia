// backend/controllers/chat/chemistryController.js (ULTRA-SIMPLIFICADO CON TOKEN MANAGER)

// ===== IMPORTACIONES NECESARIAS (REDUCIDAS 50%) =====
import { handleChemistryQuery, handleChemistryMultimodalQuery, handleChemistryQueryWithoutSaving, handleChemistryMultimodalQueryWithoutSaving } from "../../../../services/chat/ias/ingenieria/chemistryService.js";
...
/**
 * ✅ FUNCIÓN ULTRA-SIMPLIFICADA: queryChemistry
 * ELIMINADO: preValidationFailed import y lógica manual
 * REDUCCIÓN: 85% menos código usando TokenManager.handleCompleteAvaController
 */
export const queryChemistry = async (req, res) => {
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    serviceFunction: async (params) => {
      ...
      if (skipSave) {
        ...
        return await handleChemistryQueryWithoutSaving(modifiedParams);
      } else {
        return await handleChemistryQuery(params);
      }
    },
...
export const queryChemistryMultimodal = async (req, res) => {
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'chemistry_multimodal',
    defaultAvaName: 'Química',
    serviceFunction: async (params) => {
      const result = await handleChemistryMultimodalQuery(params);
      
      if (result.error) {
        logSecurityEvent('CHEMISTRY_MULTIMODAL_CONTROLLED_ERROR', 'Error controlado en consulta multimodal de química', {
...
export const queryChemistryMultimodalWithoutSaving = async (req, res) => {
  const validationErrors = validateMultimodalParams(req.body);
  const avaAccessInfo = req.accessInfo?.avaAccess || {};
  const tokenInfo = req.tokenInfo || {};
  const tokenWarning = req.tokenWarning || null;
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'chemistry_multimodal_without_saving',
    defaultAvaName: 'Química',
    serviceFunction: async (params) => {
      const result = await handleChemistryMultimodalQueryWithoutSaving(params);
      
      if (result.error) {
        logSecurityEvent('CHEMISTRY_MULTIMODAL_WITHOUT_SAVING_ERROR', 'Error controlado en consulta multimodal de química sin guardar', {
          userId: params.userId || req.user?.id_user,
          error: result.error,
          ip: req.ip
        }, 'medium');
