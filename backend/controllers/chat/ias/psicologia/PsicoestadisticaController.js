// backend/controllers/chat/ias/psicologia/PsicoestadisticaController.js (ULTRA-SIMPLIFICADO CON TOKEN MANAGER)

// ===== IMPORTACIONES NECESARIAS (REDUCIDAS 60%) =====
import { handlePsychoStatisticsQuery, handlePsychoStatisticsMultimodalQuery, handlePsychoStatisticsQueryWithoutSaving, handlePsychoStatisticsMultimodalQueryWithoutSaving } from "../../../../services/chat/ias/psicologia/psicoestadisticaService.js";
...
/**
 * ✅ FUNCIÓN ULTRA-SIMPLIFICADA: queryPsicoestadistica
 * REDUCCIÓN: 90% menos código usando TokenManager.handleCompleteAvaController
 */
export const queryPsicoestadistica = async (req, res) => {
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'psicoestadistica',
    defaultAvaName: 'Estadística y Métodos Cuantitativos en Psicología',
    serviceFunction: async (params) => {
      ...
      if (skipSave) {
        ...
        return await handlePsychoStatisticsQueryWithoutSaving(modifiedParams);
      } else {
        return await handlePsychoStatisticsQuery(params);
      }
    },
...
export const queryPsicoestadisticaMultimodal = async (req, res) => {
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'psicoestadistica_multimodal',
    defaultAvaName: 'Estadística y Métodos Cuantitativos en Psicología',
    serviceFunction: async (params) => {
      const result = await handlePsychoStatisticsMultimodalQuery(params);
      
      if (result.error) {
        logSecurityEvent('PSICOESTADISTICA_MULTIMODAL_CONTROLLED_ERROR', 'Error controlado en consulta multimodal de psicoestadística', {
...
export const queryPsicoestadisticaMultimodalWithoutSaving = async (req, res) => {
  const validationErrors = validateMultimodalParams(req.body);
  const avaAccessInfo = req.accessInfo?.avaAccess || {};
  const tokenInfo = req.tokenInfo || {};
  const tokenWarning = req.tokenWarning || null;
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'psicoestadistica_multimodal_without_saving',
    defaultAvaName: 'Estadística y Métodos Cuantitativos en Psicología',
    serviceFunction: async (params) => {
      const result = await handlePsychoStatisticsMultimodalQueryWithoutSaving(params);
      
      if (result.error) {
        logSecurityEvent('PSICOESTADISTICA_MULTIMODAL_WITHOUT_SAVING_ERROR', 'Error controlado en consulta multimodal de psicoestadística sin guardar', {
          userId: params.userId || req.user?.id_user,
          error: result.error,
          ip: req.ip
        }, 'medium');
