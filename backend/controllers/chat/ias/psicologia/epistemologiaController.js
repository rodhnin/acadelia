// backend/controllers/chat/psicologia/epistemologiaController.js (ULTRA-SIMPLIFICADO CON TOKEN MANAGER)

// ===== IMPORTACIONES NECESARIAS (REDUCIDAS 60%) =====
import { handleEpistemologyQuery, handleEpistemologyMultimodalQuery, handleEpistemologyQueryWithoutSaving, handleEpistemologyMultimodalQueryWithoutSaving } from "../../../../services/chat/ias/psicologia/epistemologiaService.js";
...
/**
 * ✅ FUNCIÓN ULTRA-SIMPLIFICADA: queryEpistemologia
 * REDUCCIÓN: 90% menos código usando TokenManager.handleCompleteAvaController
 */
export const queryEpistemologia = async (req, res) => {
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'epistemologia',
    defaultAvaName: 'Epistemología Genética',
    serviceFunction: async (params) => {
      ...
      if (skipSave) {
        ...
        return await handleEpistemologyQueryWithoutSaving(modifiedParams);
      } else {
        return await handleEpistemologyQuery(params);
      }
    },
...
export const queryEpistemologiaMultimodal = async (req, res) => {
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'epistemologia_multimodal',
    defaultAvaName: 'Epistemología Genética',
    serviceFunction: async (params) => {
      const result = await handleEpistemologyMultimodalQuery(params);
      
      if (result.error) {
        logSecurityEvent('EPISTEMOLOGIA_MULTIMODAL_CONTROLLED_ERROR', 'Error controlado en consulta multimodal de epistemología genética', {
...
export const queryEpistemologiaMultimodalWithoutSaving = async (req, res) => {
  const validationErrors = validateMultimodalParams(req.body);
  const avaAccessInfo = req.accessInfo?.avaAccess || {};
  const tokenInfo = req.tokenInfo || {};
  const tokenWarning = req.tokenWarning || null;
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'epistemologia_multimodal_without_saving',
    defaultAvaName: 'Epistemología Genética',
    serviceFunction: async (params) => {
      const result = await handleEpistemologyMultimodalQueryWithoutSaving(params);
      
      if (result.error) {
        logSecurityEvent('EPISTEMOLOGIA_MULTIMODAL_WITHOUT_SAVING_ERROR', 'Error controlado en consulta multimodal de epistemología genética sin guardar', {
          userId: params.userId || req.user?.id_user,
          error: result.error,
          ip: req.ip
        }, 'medium');
