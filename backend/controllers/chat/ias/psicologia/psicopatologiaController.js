...
/**
 * ✅ FUNCIÓN ULTRA-SIMPLIFICADA: queryPsicopatologia
 * REDUCCIÓN: 90% menos código usando TokenManager.handleCompleteAvaController
 */
export const queryPsicopatologia = async (req, res) => {
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'psicopatologia',
    defaultAvaName: 'Psicopatología',
    serviceFunction: async (params) => {
      ...
      if (skipSave) {
        ...
        return await handlePsychopathologyQueryWithoutSaving(modifiedParams);
      } else {
        return await handlePsychopathologyQuery(params);
      }
    },
...
export const queryPsicopatologiaMultimodal = async (req, res) => {
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'psicopatologia_multimodal',
    defaultAvaName: 'Psicopatología',
    serviceFunction: async (params) => {
      const result = await handlePsychopathologyMultimodalQuery(params);
      
      if (result.error) {
        logSecurityEvent('PSICOPATOLOGIA_MULTIMODAL_CONTROLLED_ERROR', 'Error controlado en consulta multimodal de psicopatología', {
...
export const queryPsicopatologiaMultimodalWithoutSaving = async (req, res) => {
  const validationErrors = validateMultimodalParams(req.body);
  const avaAccessInfo = req.accessInfo?.avaAccess || {};
  const tokenInfo = req.tokenInfo || {};
  const tokenWarning = req.tokenWarning || null;
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'psicopatologia_multimodal_without_saving',
    defaultAvaName: 'Psicopatología',
    serviceFunction: async (params) => {
      const result = await handlePsychopathologyMultimodalQueryWithoutSaving(params);
      
      if (result.error) {
        logSecurityEvent('PSICOPATOLOGIA_MULTIMODAL_WITHOUT_SAVING_ERROR', 'Error controlado en consulta multimodal de psicopatología sin guardar', {
          userId: params.userId || req.user?.id_user,
          error: result.error,
          ip: req.ip
        }, 'medium');
