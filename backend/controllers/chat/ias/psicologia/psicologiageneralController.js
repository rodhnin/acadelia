// backend/controllers/chat/ias/psicologia/psicologiageneralController.js (ULTRA-SIMPLIFICADO CON TOKEN MANAGER)

// ===== IMPORTACIONES NECESARIAS (REDUCIDAS 60%) =====
import { handlePsychologyGeneralQuery, handlePsychologyGeneralMultimodalQuery, handlePsychologyGeneralQueryWithoutSaving, handlePsychologyGeneralMultimodalQueryWithoutSaving } from "../../../../services/chat/ias/psicologia/psicologiageneralService.js";
...
/**
 * ✅ FUNCIÓN ULTRA-SIMPLIFICADA: queryPsicologiaGeneral
 * REDUCCIÓN: 90% menos código usando TokenManager.handleCompleteAvaController
 */
export const queryPsicologiaGeneral = async (req, res) => {
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'psicologiageneral',
    defaultAvaName: 'Psicología General Fundamental',
    serviceFunction: async (params) => {
      ...
      if (skipSave) {
        ...
        return await handlePsychologyGeneralQueryWithoutSaving(modifiedParams);
      } else {
        return await handlePsychologyGeneralQuery(params);
      }
    },
...
export const queryPsicologiaGeneralMultimodal = async (req, res) => {
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'psicologiageneral_multimodal',
    defaultAvaName: 'Psicología General Fundamental',
    serviceFunction: async (params) => {
      const result = await handlePsychologyGeneralMultimodalQuery(params);
      
      if (result.error) {
        logSecurityEvent('PSICOLOGIA_GENERAL_MULTIMODAL_CONTROLLED_ERROR', 'Error controlado en consulta multimodal de psicología general', {
...
export const queryPsicologiaGeneralMultimodalWithoutSaving = async (req, res) => {
  const validationErrors = validateMultimodalParams(req.body);
  const avaAccessInfo = req.accessInfo?.avaAccess || {};
  const tokenInfo = req.tokenInfo || {};
  const tokenWarning = req.tokenWarning || null;
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'psicologiageneral_multimodal_without_saving',
    defaultAvaName: 'Psicología General Fundamental',
    serviceFunction: async (params) => {
      const result = await handlePsychologyGeneralMultimodalQueryWithoutSaving(params);
      
      if (result.error) {
        logSecurityEvent('PSICOLOGIA_GENERAL_MULTIMODAL_WITHOUT_SAVING_ERROR', 'Error controlado en consulta multimodal de psicología general sin guardar', {
          userId: params.userId || req.user?.id_user,
          error: result.error,
          ip: req.ip
        }, 'medium');
