// backend/controllers/chat/psicologia/psicoanalisisController.js (ULTRA-SIMPLIFICADO CON TOKEN MANAGER)
...
export const queryPsicoanalisis = async (req, res) => {
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'psicoanalisis',
    defaultAvaName: 'Psicoanálisis',
    serviceFunction: async (params) => {
      ...
      if (skipSave) {
        ...
        return await handlePsychoanalysisQueryWithoutSaving(modifiedParams);
      } else {
        return await handlePsychoanalysisQuery(params);
      }
    },
...
export const queryPsicoanalisisMultimodal = async (req, res) => {
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'psicoanalisis_multimodal',
    defaultAvaName: 'Psicoanálisis',
    serviceFunction: async (params) => {
      const result = await handlePsychoanalysisMultimodalQuery(params);
      
      if (result.error) {
        logSecurityEvent('PSICOANALISIS_MULTIMODAL_CONTROLLED_ERROR', 'Error controlado en consulta multimodal de psicoanálisis', {
...
export const queryPsicoanalisisMultimodalWithoutSaving = async (req, res) => {
  const validationErrors = validateMultimodalParams(req.body);
  const avaAccessInfo = req.accessInfo?.avaAccess || {};
  const tokenInfo = req.tokenInfo || {};
  const tokenWarning = req.tokenWarning || null;
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'psicoanalisis_multimodal_without_saving',
    defaultAvaName: 'Psicoanálisis',
    serviceFunction: async (params) => {
      const result = await handlePsychoanalysisMultimodalQueryWithoutSaving(params);
      
      if (result.error) {
        logSecurityEvent('PSICOANALISIS_MULTIMODAL_WITHOUT_SAVING_ERROR', 'Error controlado en consulta multimodal de psicoanálisis sin guardar', {
          userId: params.userId || req.user?.id_user,
          error: result.error,
          ip: req.ip
        }, 'medium');
