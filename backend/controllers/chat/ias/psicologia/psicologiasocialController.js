// backend/controllers/chat/ias/psicologia/psicologiasocialController.js (ULTRA-SIMPLIFICADO CON TOKEN MANAGER)

// ===== IMPORTACIONES NECESARIAS (REDUCIDAS 60%) =====
import { handleSocialPsychologyQuery, handleSocialPsychologyMultimodalQuery, handleSocialPsychologyQueryWithoutSaving, handleSocialPsychologyMultimodalQueryWithoutSaving } from "../../../../services/chat/ias/psicologia/psicologiasocialService.js";
...
/**
 * ✅ FUNCIÓN ULTRA-SIMPLIFICADA: queryPsicologiaSocial
 * REDUCCIÓN: 90% menos código usando TokenManager.handleCompleteAvaController
 */
export const queryPsicologiaSocial = async (req, res) => {
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'psicologiasocial',
    defaultAvaName: 'Psicología Social',
    serviceFunction: async (params) => {
      ...
      if (skipSave) {
        ...
        return await handleSocialPsychologyQueryWithoutSaving(modifiedParams);
      } else {
        return await handleSocialPsychologyQuery(params);
      }
    },
...
export const queryPsicologiaSocialMultimodal = async (req, res) => {
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'psicologiasocial_multimodal',
    defaultAvaName: 'Psicología Social',
    serviceFunction: async (params) => {
      const result = await handleSocialPsychologyMultimodalQuery(params);
      
      if (result.error) {
        logSecurityEvent('PSICOLOGIA_SOCIAL_MULTIMODAL_CONTROLLED_ERROR', 'Error controlado en consulta multimodal de psicología social', {
...
export const queryPsicologiaSocialMultimodalWithoutSaving = async (req, res) => {
  const validationErrors = validateMultimodalParams(req.body);
  const avaAccessInfo = req.accessInfo?.avaAccess || {};
  const tokenInfo = req.tokenInfo || {};
  const tokenWarning = req.tokenWarning || null;
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'psicologiasocial_multimodal_without_saving',
    defaultAvaName: 'Psicología Social',
    serviceFunction: async (params) => {
      const result = await handleSocialPsychologyMultimodalQueryWithoutSaving(params);
      
      if (result.error) {
        logSecurityEvent('PSICOLOGIA_SOCIAL_MULTIMODAL_WITHOUT_SAVING_ERROR', 'Error controlado en consulta multimodal de psicología social sin guardar', {
          userId: params.userId || req.user?.id_user,
          error: result.error,
          ip: req.ip
        }, 'medium');
