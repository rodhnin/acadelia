// backend/controllers/chat/psicologia/neuropsicologiaController.js (ULTRA-SIMPLIFICADO CON TOKEN MANAGER)

// ===== IMPORTACIONES NECESARIAS (REDUCIDAS 60%) =====
import { handleNeuropsychologyQuery, handleNeuropsychologyMultimodalQuery, handleNeuropsychologyQueryWithoutSaving, handleNeuropsychologyMultimodalQueryWithoutSaving } from "../../../../services/chat/ias/psicologia/neuropsicologiaService.js";
...
/**
 * ✅ FUNCIÓN ULTRA-SIMPLIFICADA: queryNeuropsicologia
 * REDUCCIÓN: 90% menos código usando TokenManager.handleCompleteAvaController
 */
export const queryNeuropsicologia = async (req, res) => {
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'neuropsicologia',
    defaultAvaName: 'Neuropsicología',
    serviceFunction: async (params) => {
      ...
      if (skipSave) {
        ...
        return await handleNeuropsychologyQueryWithoutSaving(modifiedParams);
      } else {
        return await handleNeuropsychologyQuery(params);
      }
    },
...
export const queryNeuropsicologiaMultimodal = async (req, res) => {
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'neuropsicologia_multimodal',
    defaultAvaName: 'Neuropsicología',
    serviceFunction: async (params) => {
      const result = await handleNeuropsychologyMultimodalQuery(params);
      
      if (result.error) {
        logSecurityEvent('NEUROPSICOLOGIA_MULTIMODAL_CONTROLLED_ERROR', 'Error controlado en consulta multimodal de neuropsicología', {
...
export const queryNeuropsicologiaMultimodalWithoutSaving = async (req, res) => {
  const validationErrors = validateMultimodalParams(req.body);
  const avaAccessInfo = req.accessInfo?.avaAccess || {};
  const tokenInfo = req.tokenInfo || {};
  const tokenWarning = req.tokenWarning || null;
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'neuropsicologia_multimodal_without_saving',
    defaultAvaName: 'Neuropsicología',
    serviceFunction: async (params) => {
      const result = await handleNeuropsychologyMultimodalQueryWithoutSaving(params);
      
      if (result.error) {
        logSecurityEvent('NEUROPSICOLOGIA_MULTIMODAL_WITHOUT_SAVING_ERROR', 'Error controlado en consulta multimodal de neuropsicología sin guardar', {
          userId: params.userId || req.user?.id_user,
          error: result.error,
          ip: req.ip
        }, 'medium');
