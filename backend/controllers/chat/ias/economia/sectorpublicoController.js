// backend/controllers/chat/economia/sectorpublicoController.js (ULTRA-SIMPLIFICADO CON TOKEN MANAGER)

// ===== IMPORTACIONES NECESARIAS (REDUCIDAS 60%) =====
import { handlePublicSectorQuery, handlePublicSectorMultimodalQuery, handlePublicSectorQueryWithoutSaving, handlePublicSectorMultimodalQueryWithoutSaving } from "../../../../services/chat/ias/economia/sectorpublicoService.js";
...
/**
 * ✅ FUNCIÓN ULTRA-SIMPLIFICADA: queryPublicSector
 * REDUCCIÓN: 90% menos código usando TokenManager.handleCompleteAvaController
 */
export const queryPublicSector = async (req, res) => {
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'sectorpublico',
    defaultAvaName: 'Economía del Sector Público',
    serviceFunction: async (params) => {
      ...
      if (skipSave) {
        ...
        return await handlePublicSectorQueryWithoutSaving(modifiedParams);
      } else {
        return await handlePublicSectorQuery(params);
      }
    },
...
export const queryPublicSectorMultimodal = async (req, res) => {
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'sectorpublico_multimodal',
    defaultAvaName: 'Economía del Sector Público',
    serviceFunction: async (params) => {
      const result = await handlePublicSectorMultimodalQuery(params);
      
      if (result.error) {
        logSecurityEvent('SECTORPUBLICO_MULTIMODAL_CONTROLLED_ERROR', 'Error controlado en consulta multimodal de sector público', {
...
export const queryPublicSectorMultimodalWithoutSaving = async (req, res) => {
  const validationErrors = validateMultimodalParams(req.body);
  const avaAccessInfo = req.accessInfo?.avaAccess || {};
  const tokenInfo = req.tokenInfo || {};
  const tokenWarning = req.tokenWarning || null;
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'sectorpublico_multimodal_without_saving',
    defaultAvaName: 'Economía del Sector Público',
    serviceFunction: async (params) => {
      const result = await handlePublicSectorMultimodalQueryWithoutSaving(params);
      
      if (result.error) {
        logSecurityEvent('SECTORPUBLICO_MULTIMODAL_WITHOUT_SAVING_ERROR', 'Error controlado en consulta multimodal de sector público sin guardar', {
          userId: params.userId || req.user?.id_user,
          error: result.error,
          ip: req.ip
        }, 'medium');
