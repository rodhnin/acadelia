// backend/controllers/chat/economia/CalculoEconomicoController.js (ULTRA-SIMPLIFICADO CON TOKEN MANAGER)

// ===== IMPORTACIONES NECESARIAS (REDUCIDAS 60%) =====
import { handleCalculoEconomicoQuery, handleCalculoEconomicoMultimodalQuery, handleCalculoEconomicoQueryWithoutSaving, handleCalculoEconomicoMultimodalQueryWithoutSaving } from "../../../../services/chat/ias/economia/calculoeconomicoService.js";
...
/**
 * ✅ FUNCIÓN ULTRA-SIMPLIFICADA: queryCalculoEconomico  
 * REDUCCIÓN: 95% menos código usando TokenManager.handleCompleteAvaController
 */
export const queryCalculoEconomico = async (req, res) => {
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'calculoeconomico',
    defaultAvaName: 'Cálculo Económico',
    serviceFunction: async (params) => {
      ...
      if (skipSave) {
        ...
        return await handleCalculoEconomicoQueryWithoutSaving(modifiedParams);
      } else {
        return await handleCalculoEconomicoQuery(params);
      }
    },
...
export const queryCalculoEconomicoMultimodal = async (req, res) => {
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'calculoeconomico_multimodal',
    defaultAvaName: 'Cálculo Económico',
    serviceFunction: async (params) => {
      const result = await handleCalculoEconomicoMultimodalQuery(params);
      
      if (result.error) {
        logSecurityEvent('CALCULO_ECONOMICO_MULTIMODAL_CONTROLLED_ERROR', 'Error controlado en consulta multimodal de cálculo económico', {
...
export const queryCalculoEconomicoMultimodalWithoutSaving = async (req, res) => {
  const validationErrors = validateMultimodalParams(req.body);
  const avaAccessInfo = req.accessInfo?.avaAccess || {};
  const tokenInfo = req.tokenInfo || {};
  const tokenWarning = req.tokenWarning || null;
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'calculoeconomico_multimodal_without_saving',
    defaultAvaName: 'Cálculo Económico',
    serviceFunction: async (params) => {
      const result = await handleCalculoEconomicoMultimodalQueryWithoutSaving(params);
      
      if (result.error) {
        logSecurityEvent('CALCULO_ECONOMICO_MULTIMODAL_WITHOUT_SAVING_ERROR', 'Error controlado en consulta multimodal de cálculo económico sin guardar', {
          userId: params.userId || req.user?.id_user,
          error: result.error,
          ip: req.ip
        }, 'medium');
