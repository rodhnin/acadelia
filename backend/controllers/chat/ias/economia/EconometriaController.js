// backend/controllers/chat/economia/EconometriaController.js (ULTRA-SIMPLIFICADO CON TOKEN MANAGER)

// ===== IMPORTACIONES NECESARIAS (REDUCIDAS 60%) =====
import { handleEconometricsQuery, handleEconometricsMultimodalQuery, handleEconometricsQueryWithoutSaving, handleEconometricsMultimodalQueryWithoutSaving } from "../../../../services/chat/ias/economia/econometriaService.js";
...
/**
 * ✅ FUNCIÓN ULTRA-SIMPLIFICADA: queryEconometria  
 * REDUCCIÓN: 95% menos código usando TokenManager.handleCompleteAvaController
 */
export const queryEconometria = async (req, res) => {
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    serviceFunction: async (params) => {
      ...
      if (skipSave) {
        ...
        return await handleEconometricsQueryWithoutSaving(modifiedParams);
      } else {
        return await handleEconometricsQuery(params);
      }
    },
...
export const queryEconometriaMultimodal = async (req, res) => {
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'econometria_multimodal',
    defaultAvaName: 'Econometría',
    serviceFunction: async (params) => {
      const result = await handleEconometricsMultimodalQuery(params);
      
      if (result.error) {
        logSecurityEvent('ECONOMETRIA_MULTIMODAL_CONTROLLED_ERROR', 'Error controlado en consulta multimodal de econometría', {
...
export const queryEconometriaMultimodalWithoutSaving = async (req, res) => {
  const validationErrors = validateMultimodalParams(req.body);
  const avaAccessInfo = req.accessInfo?.avaAccess || {};
  const tokenInfo = req.tokenInfo || {};
  const tokenWarning = req.tokenWarning || null;
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'econometria_multimodal_without_saving',
    defaultAvaName: 'Econometría',
    serviceFunction: async (params) => {
      const result = await handleEconometricsMultimodalQueryWithoutSaving(params);
      
      if (result.error) {
        logSecurityEvent('ECONOMETRIA_MULTIMODAL_WITHOUT_SAVING_ERROR', 'Error controlado en consulta multimodal de econometría sin guardar', {
          userId: params.userId || req.user?.id_user,
          error: result.error,
          ip: req.ip
        }, 'medium');
