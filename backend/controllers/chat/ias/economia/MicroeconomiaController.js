// backend/controllers/chat/economia/MicroeconomiaController.js (ULTRA-SIMPLIFICADO CON TOKEN MANAGER)

// ===== IMPORTACIONES NECESARIAS (REDUCIDAS 60%) =====
import { handleMicroeconomicsQuery, handleMicroeconomicsMultimodalQuery, handleMicroeconomicsQueryWithoutSaving, handleMicroeconomicsMultimodalQueryWithoutSaving } from "../../../../services/chat/ias/economia/microeconomiaService.js";
...
/**
 * ✅ FUNCIÓN ULTRA-SIMPLIFICADA: queryMicroeconomia  
 * REDUCCIÓN: 95% menos código usando TokenManager.handleCompleteAvaController
 */
export const queryMicroeconomia = async (req, res) => {
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'microeconomia',
    defaultAvaName: 'Microeconomía',
    serviceFunction: async (params) => {
      ...
      if (skipSave) {
        ...
        return await handleMicroeconomicsQueryWithoutSaving(modifiedParams);
      } else {
        return await handleMicroeconomicsQuery(params);
      }
    },
...
export const queryMicroeconomiaMultimodal = async (req, res) => {
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'microeconomia_multimodal',
    defaultAvaName: 'Microeconomía',
    serviceFunction: async (params) => {
      const result = await handleMicroeconomicsMultimodalQuery(params);
      
      if (result.error) {
        logSecurityEvent('MICROECONOMIA_MULTIMODAL_CONTROLLED_ERROR', 'Error controlado en consulta multimodal de microeconomía', {
...
export const queryMicroeconomiaMultimodalWithoutSaving = async (req, res) => {
  const validationErrors = validateMultimodalParams(req.body);
  const avaAccessInfo = req.accessInfo?.avaAccess || {};
  const tokenInfo = req.tokenInfo || {};
  const tokenWarning = req.tokenWarning || null;
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'microeconomia_multimodal_without_saving',
    defaultAvaName: 'Microeconomía',
    serviceFunction: async (params) => {
      const result = await handleMicroeconomicsMultimodalQueryWithoutSaving(params);
      
      if (result.error) {
        logSecurityEvent('MICROECONOMIA_MULTIMODAL_WITHOUT_SAVING_ERROR', 'Error controlado en consulta multimodal de microeconomía sin guardar', {
          userId: params.userId || req.user?.id_user,
          error: result.error,
          ip: req.ip
        }, 'medium');
