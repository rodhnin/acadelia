// backend/controllers/chat/economia/macroeconomiaController.js (ULTRA-SIMPLIFICADO CON TOKEN MANAGER)

// ===== IMPORTACIONES NECESARIAS (REDUCIDAS 60%) =====
import { handleMacroeconomicsQuery, handleMacroeconomicsMultimodalQuery, handleMacroeconomicsQueryWithoutSaving, handleMacroeconomicsMultimodalQueryWithoutSaving } from "../../../../services/chat/ias/economia/macroeconomiaService.js";
...
/**
 * ✅ FUNCIÓN ULTRA-SIMPLIFICADA: queryMacroeconomia
 * REDUCCIÓN: 90% menos código usando TokenManager.handleCompleteAvaController
 */
export const queryMacroeconomia = async (req, res) => {
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'macroeconomia',
    defaultAvaName: 'Macroeconomía',
    serviceFunction: async (params) => {
      ...
      if (skipSave) {
        ...
        return await handleMacroeconomicsQueryWithoutSaving(modifiedParams);
      } else {
        return await handleMacroeconomicsQuery(params);
      }
    },
...
export const queryMacroeconomiaMultimodal = async (req, res) => {
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'macroeconomia_multimodal',
    defaultAvaName: 'Macroeconomía',
    serviceFunction: async (params) => {
      const result = await handleMacroeconomicsMultimodalQuery(params);
      
      if (result.error) {
        logSecurityEvent('MACROECONOMIA_MULTIMODAL_CONTROLLED_ERROR', 'Error controlado en consulta multimodal de macroeconomía', {
...
export const queryMacroeconomiaMultimodalWithoutSaving = async (req, res) => {
  const validationErrors = validateMultimodalParams(req.body);
  const avaAccessInfo = req.accessInfo?.avaAccess || {};
  const tokenInfo = req.tokenInfo || {};
  const tokenWarning = req.tokenWarning || null;
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'macroeconomia_multimodal_without_saving',
    defaultAvaName: 'Macroeconomía',
    serviceFunction: async (params) => {
      const result = await handleMacroeconomicsMultimodalQueryWithoutSaving(params);
      
      if (result.error) {
        logSecurityEvent('MACROECONOMIA_MULTIMODAL_WITHOUT_SAVING_ERROR', 'Error controlado en consulta multimodal de macroeconomía sin guardar', {
          userId: params.userId || req.user?.id_user,
          error: result.error,
          ip: req.ip
        }, 'medium');
