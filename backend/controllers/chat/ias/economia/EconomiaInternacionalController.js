...
/**
 * ✅ FUNCIÓN ULTRA-SIMPLIFICADA: queryEconomiaInternacional  
 * REDUCCIÓN: 95% menos código usando TokenManager.handleCompleteAvaController
 */
export const queryEconomiaInternacional = async (req, res) => {
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'economia_internacional',
    defaultAvaName: 'Economía Internacional',
    serviceFunction: async (params) => {
      ...
      if (skipSave) {
        ...
        return await handleInternationalEconomicsQueryWithoutSaving(modifiedParams);
      } else {
        return await handleInternationalEconomicsQuery(params);
      }
    },
...
export const queryEconomiaInternacionalMultimodal = async (req, res) => {
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'economia_internacional_multimodal',
    defaultAvaName: 'Economía Internacional',
    serviceFunction: async (params) => {
      const result = await handleInternationalEconomicsMultimodalQuery(params);
      
      if (result.error) {
        logSecurityEvent('ECONOMIA_INTERNACIONAL_MULTIMODAL_CONTROLLED_ERROR', 'Error controlado en consulta multimodal de economía internacional', {
...
export const queryEconomiaInternacionalMultimodalWithoutSaving = async (req, res) => {
  const validationErrors = validateMultimodalParams(req.body);
  const avaAccessInfo = req.accessInfo?.avaAccess || {};
  const tokenInfo = req.tokenInfo || {};
  const tokenWarning = req.tokenWarning || null;
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'economia_internacional_multimodal_without_saving',
    defaultAvaName: 'Economía Internacional',
    serviceFunction: async (params) => {
      const result = await handleInternationalEconomicsMultimodalQueryWithoutSaving(params);
      
      if (result.error) {
        logSecurityEvent('ECONOMIA_INTERNACIONAL_MULTIMODAL_WITHOUT_SAVING_ERROR', 'Error controlado en consulta multimodal de economía internacional sin guardar', {
          userId: params.userId || req.user?.id_user,
          error: result.error,
          ip: req.ip
        }, 'medium');
