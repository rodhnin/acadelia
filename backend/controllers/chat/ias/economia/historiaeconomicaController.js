// backend/controllers/chat/economia/historiaeconomicaController.js (ULTRA-SIMPLIFICADO CON TOKEN MANAGER)

// ===== IMPORTACIONES NECESARIAS (REDUCIDAS 60%) =====
import { handleEconomicHistoryQuery, handleEconomicHistoryMultimodalQuery, handleEconomicHistoryQueryWithoutSaving, handleEconomicHistoryMultimodalQueryWithoutSaving } from "../../../../services/chat/ias/economia/historiaeconomicaService.js";
...
/**
 * ✅ FUNCIÓN ULTRA-SIMPLIFICADA: queryHistoriaEconomica
 * REDUCCIÓN: 90% menos código usando TokenManager.handleCompleteAvaController
 */
export const queryHistoriaEconomica = async (req, res) => {
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'historia_economica',
    defaultAvaName: 'Historia Económica',
    serviceFunction: async (params) => {
      ...
      if (skipSave) {
        ...
        return await handleEconomicHistoryQueryWithoutSaving(modifiedParams);
      } else {
        return await handleEconomicHistoryQuery(params);
      }
    },
...
export const queryHistoriaEconomicaMultimodal = async (req, res) => {
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'historia_economica_multimodal',
    defaultAvaName: 'Historia Económica',
    serviceFunction: async (params) => {
      const result = await handleEconomicHistoryMultimodalQuery(params);
      
      if (result.error) {
        logSecurityEvent('HISTORIA_ECONOMICA_MULTIMODAL_CONTROLLED_ERROR', 'Error controlado en consulta multimodal de historia económica', {
...
export const queryHistoriaEconomicaMultimodalWithoutSaving = async (req, res) => {
  const validationErrors = validateMultimodalParams(req.body);
  const avaAccessInfo = req.accessInfo?.avaAccess || {};
  const tokenInfo = req.tokenInfo || {};
  const tokenWarning = req.tokenWarning || null;
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'historia_economica_multimodal_without_saving',
    defaultAvaName: 'Historia Económica',
    serviceFunction: async (params) => {
      const result = await handleEconomicHistoryMultimodalQueryWithoutSaving(params);
      
      if (result.error) {
        logSecurityEvent('HISTORIA_ECONOMICA_MULTIMODAL_WITHOUT_SAVING_ERROR', 'Error controlado en consulta multimodal de historia económica sin guardar', {
          userId: params.userId || req.user?.id_user,
          error: result.error,
          ip: req.ip
        }, 'medium');
