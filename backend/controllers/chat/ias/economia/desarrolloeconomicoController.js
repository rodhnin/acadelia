...
/**
 * ✅ FUNCIÓN ULTRA-SIMPLIFICADA: queryDesarrolloEconomico
 * REDUCCIÓN: 90% menos código usando TokenManager.handleCompleteAvaController
 */
export const queryDesarrolloEconomico = async (req, res) => {
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'desarrolloeconomico',
    defaultAvaName: 'Desarrollo Económico',
    serviceFunction: async (params) => {
      ...
      if (skipSave) {
        ...
        return await handleDevelopmentQueryWithoutSaving(modifiedParams);
      } else {
        return await handleDevelopmentQuery(params);
      }
    },
...
export const queryDesarrolloEconomicoMultimodal = async (req, res) => {
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'desarrolloeconomico_multimodal',
    defaultAvaName: 'Desarrollo Económico',
    serviceFunction: async (params) => {
      const result = await handleDevelopmentMultimodalQuery(params);
      
      if (result.error) {
        logSecurityEvent('DESARROLLO_ECONOMICO_MULTIMODAL_CONTROLLED_ERROR', 'Error controlado en consulta multimodal de desarrollo económico', {
...
export const queryDesarrolloEconomicoMultimodalWithoutSaving = async (req, res) => {
  const validationErrors = validateMultimodalParams(req.body);
  const avaAccessInfo = req.accessInfo?.avaAccess || {};
  const tokenInfo = req.tokenInfo || {};
  const tokenWarning = req.tokenWarning || null;
  ...
  await TokenManager.handleCompleteAvaController(req, res, {
    ...
    tokenInfo,
    tokenWarning,
    avaType: 'desarrolloeconomico_multimodal_without_saving',
    defaultAvaName: 'Desarrollo Económico',
    serviceFunction: async (params) => {
      const result = await handleDevelopmentMultimodalQueryWithoutSaving(params);
      
      if (result.error) {
        logSecurityEvent('DESARROLLO_ECONOMICO_MULTIMODAL_WITHOUT_SAVING_ERROR', 'Error controlado en consulta multimodal de desarrollo económico sin guardar', {
          userId: params.userId || req.user?.id_user,
          error: result.error,
          ip: req.ip
        }, 'medium');
