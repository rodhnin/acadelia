/**
 * messages.js OPTIMIZADO - Funciones para la API de mensajes con flujo multimodal mejorado
 * Versión optimizada SOLO para multimodal - mantiene toda la lógica existente
 */
...
async function processDocumentAndCodeFiles(documentFiles, codeFiles) {
  ...
  for (const docFile of documentFiles) {
    try {
      ...
      console.log(`✅ [OPTIMIZED] Documento procesado: ${docFile.file.name} (${mimeType})`);
    } catch (error) {
  ...
  for (const codeFile of codeFiles) {
    try {
      ...
      console.log(`✅ [OPTIMIZED] Código procesado: ${codeFile.file.name} (${mimeType})`);
    } catch (error) {
  ...
  const makeRequest = async () => {
    ...
    try {
      if (imageFiles.length > 0) {
        console.log(`🖼️ [MULTIMODAL] Procesando ${imageFiles.length} imágenes nuevas`);
        const imageContent = await processImageFiles(imageFiles);
        content.push(...imageContent);
      }

      if (documentFiles.length > 0 || codeFiles.length > 0) {
        console.log(`📄 [MULTIMODAL] Procesando ${documentFiles.length + codeFiles.length} archivos nuevos`);
        const fileContent = await processDocumentAndCodeFiles(documentFiles, codeFiles);
        content.push(...fileContent);
      }

    } catch (processingError) {
      console.error('❌ [MULTIMODAL] Error procesando archivos:', processingError);
    ...
    console.log(`📤 [MULTIMODAL] Enviando contenido completo:`, {
...
export async function sendMessageWithAttachmentsWithoutSaving(message, chatId, files, signal = null) {
  const makeRequest = async () => {
    ...
    try {
      if (imageFiles.length > 0) {
        console.log(`🖼️ [MULTIMODAL RETRY/EDIT] Procesando ${imageFiles.length} imágenes nuevas`);
        const imageContent = await processImageFiles(imageFiles);
        content.push(...imageContent);
      }

      if (documentFiles.length > 0 || codeFiles.length > 0) {
        console.log(`📄 [MULTIMODAL RETRY/EDIT] Procesando ${documentFiles.length + codeFiles.length} archivos nuevos`);
        const fileContent = await processDocumentAndCodeFiles(documentFiles, codeFiles);
        content.push(...fileContent);
      }

    } catch (processingError) {
      console.error('❌ [MULTIMODAL RETRY/EDIT] Error procesando archivos:', processingError);
      throw new Error('Error al procesar archivos: ' + processingError.message);
    }
    ...
    console.log(`📤 [MULTIMODAL RETRY/EDIT] Enviando contenido SIN GUARDAR:`, {
      items: content.length,
      tipos: content.map(c => c.type),
      texto: content.find(c => c.type === 'text')?.text?.substring(0, 50) + '...'
    });
...
export async function replaceInteraction(chatId, userMessageId, aiMessageId, userContent, aiContent) {
  try {
    ...
    console.log('🔍 [REPLACE] Contenido recibido:', {
    ...
    console.log('📤 [REPLACE] Enviando payload:', {
      userId,
    ...
    console.error('❌ [REPLACE] Error en replaceInteraction:', error);
    throw error;
  }
}
