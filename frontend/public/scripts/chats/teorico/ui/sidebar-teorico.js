...
import { createChatItem } from '../components/chat-item-teorico.js';
import { 
  createElement, 
  addClass, 
  removeClass, 
...
export function renderChatHistory(chats, performCleanup = true) {
  ...
  try {
    const { loadProblematicChats } = require('../utils/chat-error-handler-teorico.js');
  ...
  try {
    const { problematicChatIds, isChatProblematic } = require('../utils/chat-error-handler-teorico.js');
  ...
  setManagedTimeout(() => {
    ...
    try {
      const { isChatProblematic } = require('../utils/chat-error-handler-teorico.js');
    ...
    function processAndRenderChats(chats, filterFn) {
      ...
      import('../core/state-teorico.js').then(module => {
...
export function removeChatFromSidebar(chatId) {
  ...
  try {
    ...
    try {
      import('../api/chat-teorico.js').then(async module => {
