/**
 * content-processing.js - Módulo para procesamiento de contenido multimodal teorico
  ...
  _formatMultimodalContent(jsonData) {
    ...
    if (images.length > 0) {
      ...
      validImages.forEach(img => {
        html += `
          <div class="chat-image-item clickable" onclick="window.showFullImage('${this._escapeHtml(img.path)}')">
            <img src="${this._escapeHtml(img.path)}" alt="Imagen adjunta">
          </div>
        `;
      });
      html += `</div>`;
...
async handleDocumentClick(docElement) {
  ...
  console.log(`📂 Abriendo documento: ${fileName} (${fileId})`);
  ...
  try {
    ...
    try {
      ...
      acadelError("📂 ¡No pude abrir el documento!", "Acadel tuvo problemas cargando el archivo, pero no te preocupes");alert(`Error al cargar documento: ${error.message}`);
    }
  } finally {
    if (loadingNotificationId) {
      acadelCerrar(loadingNotificationId);
