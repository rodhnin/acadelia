...
import { copyToClipboard } from './clipboard-pdf.js';
...
class ResponseInteractionManager {
  ...
  /**
   * [PDF] Añade botones de interacción a un mensaje del usuario
   */
  addUserInteractionButtons(messageElement) {
    ...
    setTimeout(() => {
      ...
      console.log(`✅ [PDF] Botón de editar habilitado para: ${messageElement.dataset.messageId}`);
    }, 1000);
  ...
  /**
   * [PDF] *** FUNCIÓN CORREGIDA: Detecta si un mensaje es multimodal ***
  ...
  async isMultimodalMessage(userMessage) {
    try {
      ...
      const stateModule = await import('../core/state-pdf.js');
  ...
  /**
   * [PDF] *** FUNCIÓN CORREGIDA: Actualiza contenido preservando estructura multimodal ***
   */
  async updateUserMessageContent(messageElement, editedText) {
    if (!messageElement || editedText === undefined) return;
    ...
    try {
      ...
      if (isMultimodal) {
        ...
        if (parseMarkdownFunction) {
          ...
          const safeText = this.sanitizeAndFormatText(editedText);
      ...
      // Fallback seguro
      const formattedText = this.sanitizeAndFormatText(editedText).replace(/\n/g, '<br>');
  ...
  async submitEditedMessage(messageElement, editedText, originalText = "") {
    ...
    try {
      // Configurar estado UI igual que antes
      import('../ui/ui-manager-pdf.js').then(module => {
        if (typeof module.toggleUIState === 'function') {
          module.toggleUIState(true);
        }
      ...
      if (multimodalInfo.isMultimodal) {
        console.log('📝 [EDIT ACADEL] Mensaje multimodal detectado');
        ...
        console.log('📝 [EDIT ACADEL] Mensaje normal detectado');
  ...
  async handleStandardEdit(messageElement, aiMessage, editedText, abortController, ensureScrollUnlock, restoreUI) {
    ...
    `;
      ...
      const messagesModule = await import('../api/messages-pdf.js');
  ...
  /**
   * *** NUEVO: Maneja reintento de mensajes multimodales ***
   */
  async handleMultimodalRetry(messageElement, userMessage, multimodalInfo, abortController, ensureScrollUnlock, restoreUI) {
    ...
    try {
      ...
      if (this.isFreeUserLimitError(error)) {
        ...
        acadelError('¡Reintento multimodal complicado! 🎯💥', 'Acadel tuvo problemas reprocesando tu consulta multimodal');
      }
  ...
  /**
   * *** OPTIMIZADO: Maneja reintento de mensajes estándar ***
   */
  ...
  `;
    ...
    try {
      ...
      if (this.isFreeUserLimitError(error)) {
        ...
        // ✅ Solo para errores que NO son de tokens ni de límites de usuario
        acadelError('¡Reintento complicado! 🔄💥', 'Acadel tuvo problemas reprocesando tu consulta');
      }
  ...
  processResponseData(data) {
    ...
    console.log('🔍 [PROCESS RESPONSE] Entrada:', {
      data: data,
    ...
    try {
      if (typeof window.processServerResponse === 'function') {
        const result = window.processServerResponse(data);
        console.log('🔍 [PROCESS RESPONSE] Resultado de processServerResponse:', result);
        return result;
      }
    } catch (e) {
      console.warn('No se pudo acceder a processServerResponse original:', e);
    }
    ...
    // 🔍 DEBUG LOG - SALIDA DE FUNCIÓN
    console.log('🔍 [PROCESS RESPONSE] Salida:', {
      type: type,
      content: content,
      contentType: typeof content,
      contentLength: content?.length || 0,
      foundIn: content === data?.data ? 'data.data' :
        content === data?.answer ? 'data.answer' :
          content === data?.content ? 'data.content' :
            content === data?.message ? 'data.message' : 'otro'
    });
  ...
  addInteractionButtons(messageElement) {
    ...
    // *** BOTÓN DE RETRY SIEMPRE PRESENTE ***
    ...
    // ✅ CORRECCIÓN CRÍTICA: Usar currentTarget en lugar del closure
    addEvent(copyBtn, 'click', (e) => {
      e.stopPropagation();
      const currentMessageElement = e.currentTarget.closest('.ai-message');
      console.log('🎯 [COPY] Click en botón, mensaje encontrado:', currentMessageElement?.dataset?.messageId);
