...
import { getState, registerChat } from '../core/state-pdf.js';
import { validateUUID } from '../../../shared/validators.js';
import { renderChatHistory } from '../ui/sidebar-pdf.js';
import { updateHeaderForChat } from '../ui/header-manager-pdf.js';
import { 
  addClass, 
  removeClass,
  sanitizeText 
...
export async function loadChatHistory() {
  try {
    ...
    try {
      ...
      const { isChatProblematic, problematicChatIds, saveProblematicChatsToStorage } = await import('../utils/chat-error-handler-pdf.js');
...
async function cleanProblematicChats(chats) {
  ...
  try {
    const { isChatProblematic, removeProblematicChat } = await import('../utils/chat-error-handler-pdf.js');
