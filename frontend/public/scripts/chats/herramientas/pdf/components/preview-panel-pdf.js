 ...
 */
import { renderExam } from './exam-renderer-pdf.js';
import { renderMath } from '../../../matematico/math/mathjax-config.js';
import {
  copyToClipboard,
  copyElementContent,
  attachCopyEvents
} from '../utils/clipboard-pdf.js';
...
export function showPreviewPanel(content, type) {
  ...
  // NUEVO: Cerrar PDF panel si está abierto antes de mostrar preview panel
  const pdfPanel = document.querySelector('.pdf-panel');
  if (pdfPanel && pdfPanel.classList.contains('visible')) {
    console.log('Preview Panel: Cerrando PDF panel antes de abrir preview panel');
    // Importar y cerrar PDF panel
