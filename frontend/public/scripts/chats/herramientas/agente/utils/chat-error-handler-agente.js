...
import { handleNewChat } from '../core/chat-controller-agente.js';
import { deleteChat } from '../api/chat-agente.js';
import { showSuccess } from '../ui/ui-manager-agente.js';
import { 
  removeAllEvents, 
  setManagedTimeout, 
...
export function markChatAsProblem(chatId) {
  ...
  try {
    ...
    import('../ui/sidebar-agente.js').then(module => {
      ...
      import('../api/chat-agente.js').then(chatModule => {
...
export async function showCleanupDialog(chatId) {
  ...
  try {
    const { showConfirmation } = await import('../ui/ui-manager-agente.js');
...
if (confirm('Este chat tiene problemas técnicos. ¿Quieres que Acadel lo elimine por ti?')) {        removeProblematicChat(chatId).then(success => {
          if (getState('currentChatId') === chatId) {
            handleNewChat();
          }
        });
