...
import { API_ROUTES, MESSAGES } from '../core/config-agente.js';
import { getState, registerChat } from '../core/state-agente.js';
import { validateUUID } from '../../../shared/validators.js';
import { renderChatHistory } from '../ui/sidebar-agente.js';
import { updateHeaderForChat } from '../ui/header-manager-agente.js';
import { 
  addClass, 
  removeClass,
  sanitizeText 
...
export async function loadChatHistory() {
  try {
    ...
    const url = API_ROUTES.chatHistory(userId, herramientaId);
    ...
    try {
      ...
      const { isChatProblematic, problematicChatIds, saveProblematicChatsToStorage } = await import('../utils/chat-error-handler-agente.js');
...
async function cleanProblematicChats(chats) {
  ...
  try {
    const { isChatProblematic, removeProblematicChat } = await import('../utils/chat-error-handler-agente.js');
