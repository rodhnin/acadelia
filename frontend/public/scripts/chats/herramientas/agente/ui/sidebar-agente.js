...
import { createChatItem } from '../components/chat-item-agente.js';
import { 
  createElement, 
  addClass, 
  removeClass, 
...
export function renderChatHistory(chats, performCleanup = true) {
  ...
  try {
    const { loadProblematicChats } = require('../utils/chat-error-handler-agente.js');
  ...
  try {
    const { problematicChatIds, isChatProblematic } = require('../utils/chat-error-handler-agente.js');
  ...
  setManagedTimeout(() => {
    ...
    try {
      const { isChatProblematic } = require('../utils/chat-error-handler-agente.js');
...
export function removeChatFromSidebar(chatId) {
  ...
  try {
    ...
    try {
      import('../api/chat-agente.js').then(async module => {
