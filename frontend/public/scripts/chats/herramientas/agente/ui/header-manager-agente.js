...
import { showError, showSuccess } from './ui-manager-agente.js';
import { updateChatTitle } from '../api/chat-agente.js';
import { handleNewChat, handleDeleteChat } from '../core/chat-controller-agente.js';
...
export function closeHeaderDropdown() {
  ...
  if (dropdownContent && dropdownContent.classList.contains('show')) {
    ...
    // También quitar el atributo aria-expanded si existe
    const actionButton = elements.actionButton || document.querySelector('.action-button');
    if (actionButton) {
      removeAttribute(actionButton, 'aria-expanded');
    }
    
    // Forzar un pequeño delay para asegurar que el CSS se aplique
    setManagedTimeout(() => {
