...
class UnifiedFileAttachmentManager {
  ...
  setupDragDropListeners(elements) {
    ...
    const dropHandler = (e) => {
      ...
      console.log(`🎯 Drop event en ${this.context} con ${e.dataTransfer.files.length} archivos`);

      if (this.shouldHandleDragDrop() && e.dataTransfer.files.length > 0) {
        // CLAVE: Verificar que no estamos procesando ya
        if (!this.operationLock) {
          console.log(`✅ Procesando drop en ${this.context}`);
          this.handleDroppedFiles(e.dataTransfer.files);
        } else {
          console.log(`⏸️ Drop ignorado en ${this.context} - operación en curso`);
        }
      }
    ...
    // Document level events (solo visuales)
    const documentDragEnter = (e) => {
      e.preventDefault();
      if (e.dataTransfer.types.includes('Files') && this.shouldHandleDragDrop()) {
        elements.fileUploadContainer.classList.add('active');
      }
    };

    const documentDragLeave = (e) => {
    ...
    this.dragDropListeners = {
      ...
      documentDragEnd: documentDragEnd,
      containerDragOver: containerDragOver,
      containerDragLeave: containerDragLeave,
      containerDrop: dropHandler,
    ...
    this.eventManager.add(elements.fileUploadContainer, 'dragover', containerDragOver);
    this.eventManager.add(elements.fileUploadContainer, 'dragleave', containerDragLeave);
    this.eventManager.add(elements.fileUploadContainer, 'drop', dropHandler);
  ...
  async handleDroppedFiles(files) {
    ...
    // BLOQUEO SIMPLE para evitar procesamiento concurrente
    if (this.operationLock) {
      console.log('🦫 Acadel: Operación en curso, ignorando duplicado');
      return;
    }
    ...
    try {
      ...
      if (totalFiles > 4) { // Límite de 4 archivos
        this.showAcadelError('TOO_MANY_FILES',
          `¡Demasiados archivos! Acadel solo puede manejar 4 archivos por chat. Tienes ${currentFiles} y estás intentando agregar ${files.length} más.`);
        return;
      }
        ...
        try {
          ...
          if (currentCount >= 4) {
            this.showAcadelWarning('TOO_MANY_FILES',
              `Límite alcanzado. Acadel procesó ${processedCount} archivos, pero no puede agregar más.`);
            break;
          }
  ...
  getCurrentFileCount() {
    let selector;
    if (this.context === 'welcome') {
      selector = '#welcome-file-preview-container .file-preview[data-file-id]';
    } else {
      selector = '.file-preview-container .file-preview[data-file-id]';
    }

    const fileElements = document.querySelectorAll(selector);
    const count = fileElements.length;
    console.log(`📊 getCurrentFileCount (${this.context}): ${count} archivos encontrados`);
    return count;
  }

  removeDragDropListeners() {
    if (this.dragDropListeners) {
      // Los listeners ya se limpian automáticamente con eventManager.cleanup()
      // pero marcamos que fueron removidos
      this.dragDropListeners = null;
    }
  }
  ...
  async handleSingleFilePreview(file) {
    ...
    // ID único con prefijo según contexto
  ...
  async handleFileSelection(event, fileType) {
    ...
    if (currentFiles >= 4) {
      this.showAcadelError('TOO_MANY_FILES',
        '¡Límite alcanzado! Acadel ya tiene 4 archivos. Elimina alguno antes de agregar más.');
      event.target.value = '';
      return;
    }
  ...
  transferFilesToChat() {
    ...
    // ✅ PROTECCIÓN CONTRA TRANSFERENCIA MÚLTIPLE
    if (this._isTransferring) {
      console.log('🚫 Transferencia ya en progreso');
      return [];
    }
    ...
    try {
      ...
      // ✅ VERIFICAR SI YA HAY ARCHIVOS TEMPORALES
      if (window.temporaryWelcomeFiles) {
        console.warn('⚠️ Ya existen archivos temporales, limpiando...');
  ...
  async receiveTransferredFiles() {
    ...
    if (this._isReceiving) {
      console.log('🚫 Ya recibiendo archivos transferidos');
      return;
    }
    ...
    try {
      // ✅ COPIA LOCAL Y LIMPIEZA INMEDIATA
      const filesToProcess = [...window.temporaryWelcomeFiles];
      delete window.temporaryWelcomeFiles;

      console.log(`🎯 Recibiendo ${filesToProcess.length} archivos transferidos desde welcome`);
...
function setupEmergencyDragCleanup() {
  ...
  const cleanupDragAreas = () => {
    const areas = document.querySelectorAll('#drag-drop-area, .file-upload-container');
    areas.forEach(area => {
      if (area) {
  ...
  console.log('✅ Limpieza de emergencia para drag-drop configurada');
}
...
if (typeof window !== 'undefined') {
  ...
  window.handleDroppedFiles = function (files) {
    ...
    if (window._acadel_drop_processing) {
      console.log('🚫 Procesamiento global de drop ya en curso, ignorando duplicado');
      return;
    }

    window._acadel_drop_processing = true;
    ...
    let promise;
    if (isWelcome && welcomeFileManager) {
      console.log('🏠 Usando welcomeFileManager');
      promise = welcomeFileManager.handleDroppedFiles(files);
    } else if (!isWelcome && fileAttachmentManager) {
      console.log('💬 Usando fileAttachmentManager');
      promise = fileAttachmentManager.handleDroppedFiles(files);
    } else {
      console.warn('⚠️ No se encontró manager apropiado');
      window._acadel_drop_processing = false;
      return;
    }

    // Liberar bandera cuando termine el procesamiento
